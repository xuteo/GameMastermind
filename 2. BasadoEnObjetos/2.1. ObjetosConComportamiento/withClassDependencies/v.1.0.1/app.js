...
function initGame () {
    let that = {
        ...
        calculateResult (secretCombination, proposedCombination) {
            let results = []
            for (let combinationItem of proposedCombination.getAll()) {
                let result = this.createResultHolder();
                result.proposedCombination = combinationItem
                result.blackCount = this.countBlacks(secretCombination, combinationItem);
                result.whiteCount = this.countWhites(secretCombination, combinationItem);
    ...
    return {
        play () {
            ...
            initGamePrinter().printHeader(proposedCombination);
            do {
                proposedCombination.add(proposedCombination.request());
                initGamePrinter().printHeader(proposedCombination);
                initGamePrinter().printBoard(that.calculateResult(secretCombination, proposedCombination)); 
            } while (that.resume(secretCombination, proposedCombination));
            initGamePrinter().printFinalResult(secretCombination, proposedCombination);
        },
    };

}

function initGamePrinter () {
    return {
        printWinner () {
    ...
    return {
        request () {
            ...
            do {
                ...
                initCombinationError().printErrors(combination);
    ...
    return {
        ...
        printErrors(combination) {
            ...
            for (errorMethod in errors) {
                ...
                if(!errors[errorMethod]){console.writeln(errorLiterals[errorMethod][LiteralIndex])}
            }
        }
    }
};
